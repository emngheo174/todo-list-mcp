"use strict";Object.defineProperty(exports,Symbol.toStringTag,{value:"Module"});const p="mcpui.dev/ui-",l=`var __defProp = Object.defineProperty;
...
`;function o(e){const n=e?JSON.stringify(e):"{}";return`
<script>
(function() {
  'use strict';
...
<\/script>
`.trim()}function i(e){const n={...e.resourceProps??{}};if(e.uiMetadata||e.metadata){const t=Object.fromEntries(Object.entries(e.uiMetadata??{}).map(([a,s])=>[`${p}${a}`,s]));n._meta={...t,...e.metadata??{},...n._meta??{}}}return n}function c(e){if(typeof Buffer<"u")return Buffer.from(e,"utf-8").toString("base64");if(typeof TextEncoder<"u"&&typeof btoa<"u"){const t=new TextEncoder().encode(e);let a="";const s=8192;for(let r=0;r<t.length;r+=s)a+=String.fromCharCode(...t.slice(r,r+s));return btoa(a)}else{console.warn("MCP-UI SDK: Buffer API and TextEncoder/btoa not available. Base64 encoding might not be UTF-8 safe.");try{return btoa(e)}catch{throw new Error("MCP-UI SDK: Suitable UTF-8 to Base64 encoding method not found, and fallback btoa failed.")}}}function d(e){if(e&&e.appsSdk?.enabled)return e.appsSdk.mimeType??"text/html+skybridge"}function g(e,n){if(!n)return e;const t=[];if(n.appsSdk?.enabled){const r=o(n.appsSdk.config);t.push(r)}if(t.length===0)return e;const a=t.join(`
`);let s;return e.includes("<head>")?s=e.replace("<head>",`<head>
${a}`):e.includes("<html>")?s=e.replace("<html>",`<html>
<head>
${a}
</head>`):s=`${a}
${e}`,s}function u(e){let n,t;if(e.content.type==="rawHtml"){if(!e.uri.startsWith("ui://"))throw new Error("MCP-UI SDK: URI must start with 'ui://' when content.type is 'rawHtml'.");if(n=e.content.htmlString,typeof n!="string")throw new Error("MCP-UI SDK: content.htmlString must be provided as a string when content.type is 'rawHtml'.");e.adapters?(n=g(n,e.adapters),t=d(e.adapters)??"text/html"):t="text/html"}else if(e.content.type==="externalUrl"){if(!e.uri.startsWith("ui://"))throw new Error("MCP-UI SDK: URI must start with 'ui://' when content.type is 'externalUrl'.");if(n=e.content.iframeUrl,typeof n!="string")throw new Error("MCP-UI SDK: content.iframeUrl must be provided as a string when content.type is 'externalUrl'.");t="text/uri-list"}else if(e.content.type==="remoteDom"){if(!e.uri.startsWith("ui://"))throw new Error("MCP-UI SDK: URI must start with 'ui://' when content.type is 'remoteDom'.");if(n=e.content.script,typeof n!="string")throw new Error("MCP-UI SDK: content.script must be provided as a string when content.type is 'remoteDom'.");t=`application/vnd.mcp-ui.remote-dom+javascript; framework=${e.content.framework}`}else{const s=e.content;throw new Error(`MCP-UI SDK: Invalid content.type specified: ${s}`)}let a;switch(e.encoding){case"text":a={uri:e.uri,mimeType:t,text:n,...i(e)};break;case"blob":a={uri:e.uri,mimeType:t,blob:c(n),...i(e)};break;default:{const s=e.encoding;throw new Error(`MCP-UI SDK: Invalid encoding type: ${s}`)}}return{type:"resource",resource:a,...e.embeddedResourceProps??{}}}function m(e){window.parent&&window.parent.postMessage(e,"*")}const h={UI_MESSAGE_RECEIVED:"ui-message-received",UI_MESSAGE_RESPONSE:"ui-message-response",UI_SIZE_CHANGE:"ui-size-change",UI_LIFECYCLE_IFRAME_READY:"ui-lifecycle-iframe-ready",UI_LIFECYCLE_IFRAME_RENDER_DATA:"ui-lifecycle-iframe-render-data",UI_RAWHTML_CONTENT:"ui-html-content"},f={WAIT_FOR_RENDER_DATA:"waitForRenderData"};function I(e,n){return{type:"tool",payload:{toolName:e,params:n}}}function w(e){return{type:"prompt",payload:{prompt:e}}}function M(e){return{type:"link",payload:{url:e}}}function y(e,n){return{type:"intent",payload:{intent:e,params:n}}}function A(e){return{type:"notify",payload:{message:e}}}exports.InternalMessageType=h;exports.ReservedUrlParams=f;exports.createUIResource=u;exports.getAdapterMimeType=d;exports.getAppsSdkAdapterScript=o;exports.postUIActionResult=m;exports.uiActionResultIntent=y;exports.uiActionResultLink=M;exports.uiActionResultNotification=A;exports.uiActionResultPrompt=w;exports.uiActionResultToolCall=I;exports.wrapHtmlWithAdapters=g;
